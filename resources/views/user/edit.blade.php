...
<body class="bg-light">
    <div class="container">
      ...
      @include('components.sfAlerts')
          <h4 class="mb-3">{{__("Edit")}} {{__("User")}}</h4>
          <form method="post" action="{{route('users.update',$user->id)}}" >
            ...
            <div class="mb-3">
              ...
              <div class="input-group">
                ...
                <input type="text" name="name" value="{{old('name')??$user->name}}" class="form-control" id="username" placeholder="{{__("Username")}}" >
              </div>
              @if($errors->has('name'))
              <div>
            ...
            <div class="mb-3">
              <label for="email">{{__("Email")}}</label>
                <input type="email" name="email" value="{{old('email')??$user->email}}" class="form-control" id="email" placeholder="{{__("Email")}}" >
            </div>
            @if($errors->has('email'))
              <div>
            ...
            <div class="mb-3">
              ...
              <select class="form-select" name="is_admin" aria-label="Default select example">
                <option @if($user->is_admin ==1) selected @endif value="1">{{__("Admin")}}</option>
                <option @if($user->is_admin ==0) selected @endif value="0">{{__("User")}}</option>
              </select>
            </div>
